...
export class Hash {
   ...
   */
  static async of(sum: Uint8Array | string): Promise<Hash> {
    const buf = await crypto.subtle.digest('SHA-512', sum instanceof Uint8Array ? sum : new TextEncoder().encode(sum));
    return new Hash(new Uint8Array(buf, 0, BYTE_LENGTH));
  }
