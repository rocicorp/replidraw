    ...
    try {
      return await this._store.withWrite(async write => {
        ...
        if (mainHash) {
          ...
          if (!loaded) {
            throw new Error(
              `Corrupt database: could not find headHash: ${mainHash}`,
            );
          }
          commit = loaded;
        } else {
...
function poke(
  ...
  if (existing) {
    console.log(
      `Found existing server socket for client: ${clientID} - closing`,
    );
    existing.close();
  }
...
async function push(
  ...
  for (let mutation of pushRequest.mutations) {
    ...
    if (mutation.id > expectedMutationID) {
      return new Response(`Mutation ${mutation.id} is from the future`, {
        status: 500,
      });
    }
...
async function computePatch(
  sourceCookie: string | null,
  destCommit: LoadedCommit,
  read: Read,
  ...
  if (sourceCookie !== null) {
    ...
    if (sourceCommit) {
      ...
      console.warn(
        `Could not find cookie "${sourceCookie}" - sending reset patch`,
      );
    }
  }
  ...
  if (sourceMap === null) {
    ...
    patch.push(
      ...[...destCommit.userData.entries()].map(([key, value]) => ({
        op: 'put' as const,
        key,
...
async function sendSuperpokes(
  read: Read,
  destCookie: string | null,
  destCommit: LoadedCommit,
  sockets: Map<string, Websocket>,
): Promise<void> {
